...
#ifndef COS_BRIDGE_WEBSOCKET_SERVER_HPP
#define COS_BRIDGE_WEBSOCKET_SERVER_HPP
...
//                return RECOVERABLE;
    ...
    public:
        ...
        void publish_parameter_values(ConnHandle client_handle, const std::vector<Parameter> &parameters,
        ...
        void send_message(ConnHandle client_handle, ChannelId chan_id, uint64_t timestamp,
        ...
        void update_connection_graph(const MapOfSets &published_topics, const MapOfSets &subscribed_topics,
                                     const MapOfSets &advertised_services) override;

        void send_fetch_asset_response(ConnHandle client_handle, const FetchAssetResponse &response) override;
    ...
    private:
        ...
        void send_status_and_log_msg(ConnHandle client_handle, const StatusLevel level,
                                     const std::string &message);

        void unsubscribe_params_without_subscriptions(ConnHandle hdl,
    ...
    inline Server<ServerConfiguration>::Server(std::string name, LogCallback logger,
        ...
        _server.set_validate_handler(std::bind(&Server::validate_connection, this, std::placeholders::_1));
        _server.set_open_handler(std::bind(&Server::handle_connection_opened, this, std::placeholders::_1));
        _server.set_close_handler([this](ConnHandle hdl) {
            _handler_callback_queue->add_callback([this, hdl]() {
                this->handle_connection_closed(hdl);
            });
        });
        _server.set_message_handler([this](ConnHandle hdl, MessagePtr msg) {
            _handler_callback_queue->add_callback([this, hdl, msg]() {
    ...
    inline void Server<ServerConfiguration>::publish_parameter_values(
        ...
        std::copy_if(parameters.begin(), parameters.end(), std::back_inserter(non_empty_parameters),
                     [](const auto &p) {
    ...
    inline void Server<ServerConfiguration>::update_parameter_values(
        ...
        for (const auto &client_param_subscriptions: _client_param_subscriptions) {
            ...
            std::copy_if(parameters.begin(), parameters.end(), std::back_inserter(params_to_send_to_client),
            ...
            if (!params_to_send_to_client.empty()) {
                publish_parameter_values(client_param_subscriptions.first, params_to_send_to_client, std::nullopt);
            }
        }
    }
        ...
        if (!removed_services.empty()) {
            const auto msg =
                    Json{{"op",         "unadvertiseServices"},
                         {"serviceIds", std::move(removed_services)}}.dump();
            std::shared_lock<std::shared_mutex> clients_lock(_clients_mutex);
            for (const auto &[hdl, client_info]: _clients) {
                (void) client_info;
                send_raw_json(hdl, msg);
    ...
    template<typename ServerConfiguration>
    inline void Server<ServerConfiguration>::send_message(ConnHandle client_handle, ChannelId chan_id,
    ...
    template<typename ServerConfiguration>
    inline void Server<ServerConfiguration>::send_service_response(ConnHandle client_handle,
    ...
    inline void Server<ServerConfiguration>::update_connection_graph(
        ...
        std::unordered_set<std::string> known_topic_names, known_service_names;
        {
            ...
            for (const auto &[name, publisher_ids]: published_topics) {
                ...
                if (iter == _connection_graph.published_topics.end() ||
                    ...
                    publisher_diff.push_back(Json{{"name",         name},
            ...
            for (const auto &[name, subscriber_ids]: subscribed_topics) {
                ...
                if (iter == _connection_graph.subscribed_topics.end() ||
                    ...
                    subscriber_diff.push_back(Json{{"name",          name},
            ...
            for (const auto &[name, provider_ids]: advertised_services) {
                ...
                if (iter == _connection_graph.advertised_services.end() ||
                    ...
                    services_diff.push_back(Json{{"name",        name},
        ...
        std::copy_if(known_topic_names.begin(), known_topic_names.end(), std::back_inserter(removed_topics),
    ...
    inline void Server<ServerConfiguration>::send_fetch_asset_response(
        ...
        const size_t data_size = response.status == FetchAssetStatus::Success ? response.data.size() : 0ul;
    ...
    template<typename ServerConfiguration>
    inline std::string Server<ServerConfiguration>::remote_endpoint_string(cos_bridge_base::ConnHandle client_handle) {
        websocketpp::lib::error_code ec;
        const auto con = _server.get_con_from_hdl(client_handle, ec);
    ...
    inline bool Server<ServerConfiguration>::validate_connection(ConnHandle hdl) {
        ...
        _server.get_alog().write(APP, "Rejecting client " + remote_endpoint_string(hdl) +
    ...
    template<typename ServerConfiguration>
    inline void Server<ServerConfiguration>::handle_connection_opened(cos_bridge_base::ConnHandle hdl) {
        auto con = _server.get_con_from_hdl(hdl);
        const auto endpoint = remote_endpoint_string(hdl);
        {
            ...
            if (_clients.size() > MAX_CLIENT_COUNT) {
                _server.get_alog().write(APP, "Client " + endpoint
                                              +
    ...
    inline void Server<ServerConfiguration>::handle_connection_closed(ConnHandle hdl) {
        ...
        bool was_subscribed_to_connection_graph;
        {
            ...
            if (client_iter == _clients.end()) {
                _server.get_elog().write(RECOVERABLE, "Client " + remote_endpoint_string(hdl) +
        ...
        for (const auto client_channel_id: old_advertised_channels) {
            _server.get_alog().write(APP, "Client " + client_name + " unadvertising channel " +
    ...
    inline void Server<ServerConfiguration>::unsubscribe_params_without_subscriptions(
        ...
        {
            std::lock_guard<std::mutex> lock(_client_param_subscriptions_mutex);
            std::copy_if(param_names.begin(), param_names.end(), std::back_inserter(params_to_unsubscribe),
        ...
        if (_handlers.parameter_subscription_handler && !params_to_unsubscribe.empty()) {
            ...
            try {
                _handlers.parameter_subscription_handler(params_to_unsubscribe,
    ...
    template<typename ServerConfiguration>
    inline bool Server<ServerConfiguration>::is_parameter_subscribed(const std::string &param_name) const {
        return std::find_if(_client_param_subscriptions.begin(), _client_param_subscriptions.end(),
    ...
    template<typename ServerConfiguration>
    inline bool Server<ServerConfiguration>::has_handler(uint32_t
                                                         op) const {
        switch (op) {
            case SUBSCRIBE:
                return bool(_handlers.subscribe_handler);
            case UNSUBSCRIBE:
                return bool(_handlers.unsubscribe_handler);
            case ADVERTISE:
                return bool(_handlers.client_advertise_handler);
            case UNADVERTISE:
                return bool(_handlers.client_unadvertise_handler);
            case GET_PARAMETERS:
                return bool(_handlers.parameter_request_handler);
            case SET_PARAMETERS:
                return bool(_handlers.parameter_change_handler);
            case SUBSCRIBE_PARAMETER_UPDATES:
            case UNSUBSCRIBE_PARAMETER_UPDATES:
                return bool(_handlers.parameter_subscription_handler);
            case SUBSCRIBE_CONNECTION_GRAPH:
            case UNSUBSCRIBE_CONNECTION_GRAPH:
                return bool(_handlers.subscribe_connection_graph_handler);
            case FETCH_ASSET:
                return bool(_handlers.fetch_asset_handler);
            default:
    ...
    void Server<ServerConfiguration>::handle_subscribe(const Json &payload, ConnHandle hdl) {
        ...
        const auto find_subscription_by_sub_id =
                [](const std::unordered_map<ChannelId, SubscriptionId> &subscriptions_by_channel,
                   SubscriptionId sub_id) {
                    return std::find_if(subscriptions_by_channel.begin(), subscriptions_by_channel.end(),
    ...
    void Server<ServerConfiguration>::handle_advertise(const Json &payload, ConnHandle hdl) {
        ...
        for (const auto &chan: payload.at("channels")) {
            ...
            if (!is_first_publication && client_publications.find(channel_id) != client_publications.end()) {
                send_status_and_log_msg(hdl, StatusLevel::Error,
    ...
    template<typename ServerConfiguration>
    void Server<ServerConfiguration>::handle_subscribe_parameter_updates(const Json &payload,
                                                                         ConnHandle hdl) {
        ...
        std::vector<std::string> params_to_subscribe;
        {
            ...
            std::lock_guard<std::mutex> lock(_client_param_subscriptions_mutex);
            std::copy_if(param_names.begin(), param_names.end(), std::back_inserter(params_to_subscribe),
        ...
        if (!params_to_subscribe.empty()) {
            _handlers.parameter_subscription_handler(params_to_subscribe,
                                                     ParameterSubscriptionOperation::SUBSCRIBE, hdl);
        }
    }

    template<typename ServerConfiguration>
    void Server<ServerConfiguration>::handle_unsubscribe_parameter_updates(const Json &payload,
                                                                           ConnHandle hdl) {
        const auto param_names = payload.at("parameterNames").get<std::unordered_set<std::string >>();
        {
            std::lock_guard<std::mutex> lock(_client_param_subscriptions_mutex);
    ...
    void Server<ServerConfiguration>::handle_subscribe_connection_graph(ConnHandle hdl) {
        ...
        Json::array_t published_topics_json, subscribed_topics_json, advertised_services_json;
        {
            ...
            for (const auto &[name, ids]: _connection_graph.published_topics) {
                published_topics_json.push_back(Json{{"name",         name},
                                                     {"publisherIds", ids}});
            }
            for (const auto &[name, ids]: _connection_graph.subscribed_topics) {
                subscribed_topics_json.push_back(Json{{"name",          name},
                                                      {"subscriberIds", ids}});
            }
            for (const auto &[name, ids]: _connection_graph.advertised_services) {
                advertised_services_json.push_back(Json{{"name",        name},
                                                        {"providerIds", ids}});
            }
        }
        ...
        try {
            switch (string_hash(op)) {
                ...
                case FETCH_ASSET:
                    ...
                    break;
                default:
                    send_status_and_log_msg(hdl, StatusLevel::Error, "Unrecognized client opcode \"" + op + "\"");
                    break;
            }
        ...
        switch (op) {
            case ClientBinaryOpcode::MESSAGE_DATA: {
                ...
                try {
                    ...
                    send_status_and_log_msg(hdl, StatusLevel::Error, "callService: Failed to execute handler");
                }
            }
                break;
    ...
    template<typename ServerConfiguration>
    inline void Server<ServerConfiguration>::send_status_and_log_msg(ConnHandle client_handle,
    ...
    template<typename ServerConfiguration>
    inline void Server<ServerConfiguration>::send_binary(ConnHandle hdl, const uint8_t *payload,
                                                         size_t payload_size) {
        try {
...
#endif //COS_BRIDGE_WEBSOCKET_SERVER_HPP
