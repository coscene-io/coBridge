...
/// @return A tuple holding goal, result and feedback definitions
    ...
    MessageSpec::MessageSpec(MessageDefinitionFormat format, std::string text,
                             const std::string &package_context)
            : dependencies(parse_dependencies(format, text, package_context)), text(std::move(text)), format(format) {}
    ...
    const MessageSpec &MessageDefinitionCache::load_message_spec(
        ...
        const auto lines_iter = std::find_if(lines.begin(), lines.end(), [&filename](const std::string &line) {
        ...
        // Read the file
        const std::string full_path = share_dir + std::filesystem::path::preferred_separator + *lines_iter;
