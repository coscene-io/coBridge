...
int main(int argc, char *argv[]) {
    ...
    size_t num_threads = 0;
    {
        ...
        dummy_node->declare_parameter(num_threads_description.name, DEFAULT_NUM_THREADS,
                                      num_threads_description);
        num_threads = static_cast<size_t>(dummy_node->get_parameter(num_threads_description.name).as_int());
    }
