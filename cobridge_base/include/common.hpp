...
namespace cobridge_base {
    ...
    struct ClientMessage {
        ...
        ClientMessage(uint64_t log_time, uint64_t publish_time, uint32_t sequence,
                      const ClientAdvertisement &advertisement, size_t data_length, const uint8_t *raw_data)
                : log_time(log_time), publish_time(publish_time), sequence(sequence), advertisement(advertisement),
    ...
    struct ServiceResponse {
        ...
        bool operator==(const ServiceResponse &other) const {
            return service_id == other.service_id && call_id == other.call_id && encoding == other.encoding &&
                   serv_data == other.serv_data;
        }
    };
